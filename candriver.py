# source: https://github.com/wojciech-kochanski/raspberry-pico-can-bus/blob/main/candriver.py
...
class CanFrame:
    ...
    def __init__(self, id=0, rtr=False, extended=False, payload=[]):
        self.id = id
        self.rtr = rtr
    ...
    def __str__(self):
        ...
        return "ID: 0x%x \ttype: %s \tdlc: %d \tpayload: [%s] \t%s" % (
            self.id,
            "Extended" if self.extended else "Normal",
            self.dlc,
            payload_str,
            "Remote Transmission Request" if self.rtr else "",
        )
...
class MCP2515:
    ...
    def Receive(self):
        ...
        if extended:
            identifier = 0
            id_buff = self.ReadByte(RXB0EID0) + (
                self.ReadByte(RXB0EID8) << 8
            )  # EID15:0
            ...
            rtr = (self.ReadByte(RXB0DLC) >> 6) & 0x1
        else:
        ...
        frame = CanFrame(id=identifier, extended=extended, rtr=rtr, payload=CAN_RX_Buf)
        return frame
...
if __name__ == "__main__":
    ...
    uptime_frame = CanFrame(id=123, payload=[0])
    tick_counter = 0
